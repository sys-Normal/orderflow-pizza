...
**배경:** 판매자 주문 목록에 메뉴명/거절 버튼을 추가하던 중, 판매자가 매장을 여러 개 소유할 수 있는 스키마 구조(`Store.ownerId`에 unique 제약 없음)가 눈에 띔. 확인해보니 실제로 도달 가능한 버그는 아니고 시드 데이터 특성(`seller@orderflow.pizza` 1개 계정에 매장 10개를 몰아둠)이었지만, "판매자 1명 = 매장 1개"가 맞는 비즈니스 규칙이라는 데 합의함. 또한 조사 중 앱 전체에 매장을 실제로 생성하는 흐름이 seed 스크립트 말고는 전혀 없다는 것도 확인됨. 판단/논의 상세 근거는 [decision-log.md](decision-log.md)의 2026-07-24 항목 참고.

**완료(P0~매장 등록 플로우):**
- `Store.ownerId`에 `@unique` 제약 추가 + 마이그레이션.
- 시드 데이터는 원래도 판매자 1명·매장 1개만 만들고 있어 재작성 불필요했음 — 대신 이 제약을 깨던 런타임 매장 자동복제 기능(`seed-nearby.ts`)이 원인이라 제거함.
- **매장 등록 플로우**: 처음엔 "판매자가 신청 → pending → 관리자 승인" 구조로 짰다가, 이 브랜드가 여러 독립 사업자가 입점하는 마켓플레이스가 아니라 "OrderFlow Pizza" 단일 브랜드의 여러 지점이라는 논의 끝에 **관리자가 지점을 생성하고 로그인 계정(이메일/임시비번)을 발급해 지점주에게 전달**하는 구조로 재설계함(`/admin/stores/new`). 셀프 가입은 없고, 생성된 지점은 `status: approved`로 즉시 시작 — 이제 pending 승인 대기열이 없으므로 아래 "매장 목록/요청 관리 탭 분리" 계획은 더 이상 유효하지 않음(이미 지운 항목).
- 위치 입력은 주소 검색(Nominatim, 미리보기→선택 확정) + 지도 클릭 조정으로 구현.
...
2. **P3 — 재고/용량 관리**: 메뉴 재고, 동시 처리 가능 주문 수 임계치 등. 데이터 모델부터 새로 설계해야 하는 별도의 큰 기획이라 이번 스코프에서 제외, 백로그로만 유지.
3. **P4 — 판매자/관리자 로그인·라우트·API 완전 분리**: 보류 상태. 지금 결정하지 않고, 착수할 때 그 시점까지의 진행 상태를 보고 이어서 논의하기로 함.
