# 백로그

아직 착수하지 않은 작업 후보를 남겨두는 문서. 착수하면 항목을 지우고 실제 커밋/이슈로 옮긴다.

---

## 실시간 주문 상태 추적 (판매자 라이브 피드 + 구매자 상태 조회)

**배경:** 현재 [/admin/orders](../src/app/(admin)/admin/orders/page.tsx)(`OrderList`)는 과거 주문을 훑어보는 히스토리 조회 화면에 가깝다. 실제 매장 운영 중에는 새 주문이 들어올 때마다 화면 위로 쌓이는 라이브 피드가 더 우선순위가 높다는 논의가 있었음. 히스토리 목록 자체는 계속 필요하니, 없애기보다 별도 화면(또는 같은 화면 내 모드 전환)으로 추가하는 방향.

추가로, 판매자 쪽엔 주문 상태 변경 기능([order-status-buttons.tsx](../src/components/order-status-buttons.tsx), 접수됨→조리중→준비완료→완료)이 이미 있는데 구매자 쪽엔 그 상태를 볼 수 있는 화면이 전혀 없다는 게 확인됨([/confirmation/[orderId]](../src/app/(customer)/confirmation/[orderId]/page.tsx)에 상태 표시 없음, "내 주문" 목록 페이지도 없음). 판매자 라이브 피드와 구매자 상태 추적은 결국 같은 실시간 인프라가 필요한 같은 작업이라 하나로 묶어서 진행.

**구현 방향(논의 결론):**
- 폴링은 실시간성이 부족해 제외.
- 업계(배민/쿠팡이츠 등)는 매장 상시 접속 기기에서 WebSocket(또는 MQTT over WS) 상시 연결 + OS 푸시(FCM/APNs)를 끊김 대비 백업으로 병행하고, 알람음은 클라이언트가 로컬로 재생하는 구조가 일반적.
- 다만 이 프로젝트는 실시간 인프라가 전무하고 포트폴리오 규모라 풀 WebSocket 서버는 과함. Next.js route handler로 구현 가능한 **SSE(Server-Sent Events)** 로 서버→클라이언트 단방향 푸시부터 시작하는 게 적합해 보임.
- SSE는 자원 소모가 거의 없고(연결 하나당 열린 HTTP 응답일 뿐), Redis 같은 별도 메시지 브로커 없이 **같은 Node 프로세스 안의 `EventEmitter` 싱글턴**으로 충분— 이 프로젝트가 SQLite(better-sqlite3)를 쓰는 것 자체가 이미 "상시 구동되는 단일 프로세스" 배포를 전제로 하므로(Vercel 같은 서버리스와는 애초에 안 맞음), 원격 서버 1대에 단일 프로세스로 올려도 여러 사용자가 동시에 실시간으로 잘 받는다. 나중에 트래픽이 커져 여러 프로세스로 수평 확장할 때만 Redis pub/sub 같은 걸로 교체하면 됨 — 지금 구조를 크게 갈아엎을 필요는 없음.

**구현 순서(가이드):**
1. 서버 내 이벤트 버스: `src/lib/events.ts`에 Node `EventEmitter` 싱글턴 + `emitOrderEvent(storeId, order)` 같은 헬퍼 함수.
2. Server Action에서 이벤트 발행: `createOrder`, `updateOrderStatus`, `updateStoreStatus` 성공 직후 해당 헬퍼 호출.
3. SSE 라우트 핸들러 2개:
   - `/api/stores/[storeId]/orders/stream` — 판매자/관리자용. 신규 주문 + 상태 변경을 실시간으로 받아 라이브 피드에 반영.
   - `/api/orders/[orderId]/stream` — 구매자용. 본인 주문 상태 변경만 구독.
   - 두 곳 다 세션 확인 후 권한 없는 스트림 구독은 차단(seller는 본인 매장만, buyer는 본인 주문만).
4. 클라이언트 훅: `EventSource`로 구독하고 메시지 수신 시 로컬 상태 갱신. 재연결 시(브라우저가 자동 재연결) 최신 스냅샷을 한 번 다시 조회해 유실 구간 보정.
5. UI 반영:
   - 판매자: `/admin/orders`에 신규 주문이 위로 쌓이는 라이브 피드 뷰 추가(히스토리 목록과는 별도 화면 또는 탭 전환).
   - 구매자: `/confirmation/[orderId]`에 실시간 상태 배지 추가, 필요하면 "내 주문" 목록 페이지 신설.
6. 원격 배포 시 체크리스트:
   - SQLite가 디스크에 의존하므로 Vercel/Netlify 같은 서버리스 플랫폼은 배제. VPS(오라클 클라우드 프리티어, Lightsail 등) + PM2/systemd, 또는 영구 디스크를 지원하는 Fly.io/Railway/Render 같은 PaaS 사용.
   - 앱은 반드시 **단일 프로세스**로만 구동(클러스터 모드/멀티 레플리카 금지) — EventEmitter가 프로세스 메모리에 있어서 여러 프로세스로 나뉘면 이벤트가 서로 안 보임.
   - nginx 등 리버스 프록시를 앞에 둘 경우 `proxy_buffering off`로 SSE 응답 버퍼링을 꺼야 하고, 프록시/로드밸런서의 idle timeout이 SSE 연결을 끊지 않도록 충분히 길게(또는 keep-alive 핑 전송).

**미정 사항:**
- 히스토리 목록과 라이브 피드 화면을 분리할지, 필터/탭으로 한 화면에서 전환할지.
- 신규 주문 알림 시 알람음/시각적 강조 방식.
- 구매자 "내 주문" 목록 페이지를 이번에 같이 만들지, 확인 페이지 실시간 배지만 우선 넣을지.

---

## 판매자-매장 1:1 모델 강제 + 매장 등록 플로우

**배경:** 판매자 주문 목록에 메뉴명/거절 버튼을 추가하던 중, 판매자가 매장을 여러 개 소유할 수 있는 스키마 구조(`Store.ownerId`에 unique 제약 없음)가 눈에 띔. 확인해보니 실제로 도달 가능한 버그는 아니고 시드 데이터 특성(`seller@orderflow.pizza` 1개 계정에 매장 10개를 몰아둠)이었지만, "판매자 1명 = 매장 1개"가 맞는 비즈니스 규칙이라는 데 합의함. 또한 조사 중 앱 전체에 매장을 실제로 생성하는 흐름이 seed 스크립트 말고는 전혀 없다는 것도 확인됨 — `StoreStatus.pending`이 스키마엔 있지만 누가 pending 매장을 만드는지에 대한 실제 플로우가 없음(2026-07-07 결정: `/admin/stores`를 "매장 목록"/"요청 관리" 탭으로 나누자는 계획도 이 등록 플로우가 없어서 착수 못 하고 있던 상태). 판단/논의 상세 근거는 [decision-log.md](decision-log.md)의 2026-07-24 항목 참고.

**진행 순서(합의됨, 위에서부터 순서대로 — 완료 시 항목을 지우고 다음으로):**

1. **P0 — 스키마에 1:1 제약 강제**: `prisma/schema.prisma`의 `Store` 모델 `ownerId`에 `@unique` 추가 + 마이그레이션.
2. **P1 — 시드 데이터 재구성**: `prisma/seed.ts`를 판매자 1명당 매장 1개가 되도록 재작성(현재 10개 매장 → 판매자 계정 10개로 분산). [SeedAccountHint](../src/components/seed-account-hint.tsx) 로그인 힌트 UI도 계정이 늘어난 것에 맞춰 재검토(전부 나열/대표 1개만/다른 방식 — 미정).
3. **매장 등록 플로우 신설**: 판매자가 매장을 신청 → `pending` 상태로 생성 → 관리자가 승인/거절. 기존에 미뤄뒀던 "매장 목록/요청 관리 탭 분리"(2026-07-07 결정) 작업과 자연히 이어짐. A-Z 테스트(신규 판매자 가입 → 매장 등록 → 승인 → 주문 수신)를 위해 필요하다고 판단해 이번에 우선순위에 포함시킴. 스코프가 커서 착수 시 별도로 세부 계획을 다시 쪼갤 것.
4. **P2 — 목록/프로필 UX 마무리**:
   - 관리자 `/admin/orders/search`는 매장 필터를 붙일 계획이 있어 주문에 매장 정보를 계속 유지(현재도 유지되고 있는지 확인 필요).
   - 판매자 쪽은 목록에 매장명을 노출하지 않고, 프로필/헤더 등에서 "현재 소속 매장"을 확인 용도로만 표시(어필 목적 아님).
   - 주문 목록에서 "접수됨"(미확정) 상태는 기본 펼침, 그 외 상태는 기본 접힘.
5. **P3 — 재고/용량 관리**: 메뉴 재고, 동시 처리 가능 주문 수 임계치 등. 데이터 모델부터 새로 설계해야 하는 별도의 큰 기획이라 이번 스코프에서 제외, 백로그로만 유지.
6. **P4 — 판매자/관리자 로그인·라우트·API 완전 분리**: 보류 상태. 지금 결정하지 않고, 착수할 때 그 시점까지의 진행 상태를 보고 이어서 논의하기로 함.

**진행 방식(합의됨):** 우선순위 높은 순으로 한 블록씩 진행 → QA → 완료 확인 후 다음 블록으로. 각 블록 시작 전엔 사용자 확인("그렇게 진행하자")을 받고 착수.

---

## 지점 등록 위치 검색을 juso.go.kr(도로명주소) API로 전환

**배경:** [/admin/stores/new](../src/app/(admin)/admin/stores/new/page.tsx)의 위치 검색([store-location-picker.tsx](../src/components/store-location-picker.tsx))은 현재 OpenStreetMap Nominatim으로 주소→좌표 변환을 하고 있음. 결과 라벨은 표시용으로 간결하게 다듬었지만([forward-geocode.ts](../src/lib/stores/forward-geocode.ts)), 데이터 자체는 여전히 OSM 소스라 한국 도로명주소 체계와 정확히 안 맞는 경우가 있을 수 있음. 사용자가 이 지도/도로명주소 부분은 앞으로도 한 번 이상 다시 손볼 가능성이 높다고 보지만, 지금은 메인 서비스 스코프가 아니라 우선순위를 낮춰 백로그에만 남겨둠.

**전환 시 필요한 것(확인됨):**
- [business.juso.go.kr](https://business.juso.go.kr)에서 API 키를 직접 신청/발급해야 함 — 에이전트가 대신 가입할 수 없는 외부 계정 작업.
- 도로명주소 검색 API 자체는 주소 텍스트만 주고 좌표를 안 줌. 좌표까지 받으려면 "사업자용" API(좌표제공 포함)를 써야 하는데, 이건 사업자등록번호가 필요할 수 있음 — 포트폴리오 규모라 해당 안 될 가능성 있음.
- 좌표가 EPSG:5179(TM) 체계로 내려오므로 WGS84 위경도로 변환하는 로직(예: proj4)이 추가로 필요.

**미정 사항:** 사업자용 API 신청 가능 여부 확인 전까지는 착수 불가. 착수 시점에 그때 상황 보고 진행 여부 재논의.
